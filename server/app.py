...
def show_session(key):
    ...
    # Only set initial values if the session key doesn't exist yet,
    # so values persist across requests from the same client
    ...
    # Increment the visit counter each time /sessions/count is requested
    if key == "count":
        session["count"] += 1
...
def follow_crumbs():
    ...
    # Manually-set cookie: plain text, visible and editable in dev tools,
    # unlike Flask's signed session cookie
